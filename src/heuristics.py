...
def river_shortest_path(env):
    ...
    try:
        ...
        goal_location = first_variable if first_variable.var_type == 'location' else second_variable
...
def build_hv(env, lamb):
    data = None if env.spec.id not in value_heuristic_data_functions else value_heuristic_data_functions[
        env.spec.id](env)
...
def build_hp(env):
    data = None if env.spec.id not in prob_heuristic_data_functions else prob_heuristic_data_functions[
        env.spec.id](env)
